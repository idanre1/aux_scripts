...
def _getSortedFiles(allFiles, upTo=None):
...
def notebook2script(fname=None, allFiles=None, upTo=None):
    ...
    if not allFiles: _notebook2scriptSingle(fname)
    else:
        print('Begin...')
        [_notebook2scriptSingle(f) for f in _getSortedFiles(allFiles,upTo)]
        print('...End')
        
        
def _notebook2scriptSingle(fname):
    "Finds cells starting with `#export` and puts them into a new module"
    fname = Path(fname)
